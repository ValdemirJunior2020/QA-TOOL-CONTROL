...
function cleanupJobs() {
  ...
  for (const [runId, job] of jobs.entries()) {
    if ((job.status === 'completed' || job.status === 'failed') && Number(job.finishedAt || 0) < jobCutoff) {
      jobs.delete(runId)
    }
  }
  ...
  if (req.method === 'POST' && requestUrl.pathname === '/api/auto-qa/upload/start') {
    try {
      ...
      if (!Number.isInteger(totalChunks) || totalChunks < 1 || totalChunks > MAX_UPLOAD_CHUNKS) {
        throw new Error(`Invalid chunk count. Maximum is ${MAX_UPLOAD_CHUNKS}.`)
      }
...
server.listen(PORT, HOST, () => {
  ...
  console.log('Large call audio uploads are accepted in safe chunks before the job is queued.')
  startWorker()
})
