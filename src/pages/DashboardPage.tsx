...
export function DashboardPage({ user, reviews, users, onNewReview, onRefresh, refreshing, onCreateBackup, onRestoreLatestBackup }: DashboardPageProps) {
  ...
  return <div className="page-stack">
    <section className="hero-panel"><div><p className="eyebrow">Welcome back, {user.displayName}</p><h1>Keep every QA review accurate and under control.</h1><p>Live metrics, email tracking, protected saves, reporting, and backups are all connected to Agents Reviewed.</p></div><div className="hero-actions">{user.permissions.canSubmitReviews && <button className="primary-button" onClick={onNewReview}>Start a QA Review</button>}<button className="secondary-button" onClick={onRefresh} disabled={refreshing}>{refreshing ? 'Refreshing…' : 'Refresh Sheet Data'}</button></div></section>
    <section className="stat-grid dashboard-expanded-stats">
      ...
      <article className="stat-card"><span>Total Reviews</span><strong>{reviews.length}</strong><small>Loaded from Agents Reviewed</small></article>
    ...
    {user.role === 'admin' && <section className="panel backup-panel"><div><p className="eyebrow">Safety</p><h2>Backup & Restore</h2><p className="muted">Creates protected copies of Agents Reviewed, QA App Users, QA App Settings, QA App Audit, and Emails Sent Details.</p></div><div className="hero-actions"><button className="secondary-button" onClick={() => void onCreateBackup()}>Create Backup Now</button><button className="danger-button" onClick={() => void onRestoreLatestBackup()}>Restore Latest Backup</button></div></section>}
